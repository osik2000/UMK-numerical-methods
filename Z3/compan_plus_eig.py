...
from numpy import linalg, isreal, real
from scipy.linalg import companion

EPSILON = 0.0000000001

allRoots = False  # if true -> function companPlusEig will show all roots in complex form.


def companPlusEig(p):
    values, vectors = linalg.eig(companion(p))
...
if len(argv) > 1:
    companPlusEig([float(val) for val in literal_eval(argv[1])])
else:
    print("Prosze podac argumenty funkcji!\n\n"
          "Przykladowe wywolanie:\n"
          "python compan_plus_eig.py [1,2,3]")
